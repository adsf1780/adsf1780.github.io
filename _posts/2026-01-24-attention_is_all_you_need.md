...
# 기타
- 이 논문의 제목은 2017년 당시 NLP 분야를 지배하고 있던 두 가지 구조인 recurrence와 convolution이 더 이상 필요 없다고 선언하는 의미다. transformer는 두 구조의 한계였던 sequential과 locality를 해결했다는 의의를 지닌다.
...
- ensemble 기법
앙상블 기법은 여러 개의 모델을 따로 학습시킨 뒤, 그들의 예측 결과를 합쳐서 더 좋은 성능을 내는 기법이다. 이 논문에서는 여러 개의 모델을 학습시키는 대신에 선정된 몇 개의 checkpoint를 평균내는 방식을 사용했다.
![](\assets\images\2026-01-24-attention_is_all_you_need\image8.png){: width="70%" .align-center}

- 토큰화 + 임베딩 전체 과정

    텍스트: AI is smart
    1. 토큰화 - 긴 텍스트를 컴퓨터가 처리할 수 있는 최소 의미 단위(token)로 자른다.  $ \rightarrow $ ["AI", "is", "smart"]
    2. token에 ID 부여 - 컴퓨터는 문자열("AI")을 그대로 계산할 수 없기 때문에 vocabulary를 보고 각 토큰을 정수로 바꾼다. $\to$ AI = 341번, is = 10번, smart = 892번 $\to$ [341, 10, 892]
    3. 임베딩 - 이 정수 번호를 실수로 이루어진 고차원 벡터로 변환한다. 특정 vocabulary와 호환되는 learned embedding이 있는 경우에는 이 정수 번호를 인덱스로 사용하여 embedding vector를 얻을 수 있다.

- dropout
        
    딥러닝 모델의 일반화(generalization) 성능을 높이기 위해 사용하는 대표적인 정규화(regularization) 기법이다. 학습 과정에서 각 레이어의 뉴런을 설정된 확률 p에 따라 무작위로 비활성화하는 기법이다. dropout이 효과적인 이유는 다음과 같다. 첫 번째로 뉴런 간의 상호의존성을 방지한다. 특정 뉴런들이 서로 강하게 결합하여 특정 feature에만 의존하는 공동 적응(co-adaptation)을 막는다. 어떤 뉴런이 사라질지 모르기 때문에 각 뉴런은 더 견고하고 독립적인 feature를 추출하도록 강제된다. 두 번째로 앙상블과 유사한 효과를 거둘 수 있다. n개의 뉴런이 있을 때 이론적으로 $2^n$ 개의 서로 다른 부분적인 신경망을 가질 수 있다. 이 수많은 네트워크를 동시에 학습하는 효과를 내며, 테스트 시에는 이들을 평균 내어 사용하는 앙상블 효과를 거두게 된다. 학습 시에 p의 확률로 뉴런이 사라지므로, 레이어 출력의 기댓값은 1-p배로 줄어든다. 따라서 테스트 시에 모든 뉴런을 사용하기 때문에 학습 때보다 출력이 커지는 문제가 발생한다. 이를 해결하기 위해 과거에는 가중치에 1-p를 곱해 스케일을 맞췄으나, 최근에는 inverted dropout 방식을 주로 사용한다. inverted dropout은 학습 단계에서 아예 출력을 1-p로 나누어 미리 스케일을 키워두는 방식이다. PyTorch의 nn.Dropout이 이 방식을 채택하고 있다.
